...
indem man eine Tangente zu der Kurve der Funktion an diesem Punkt zieht und
...
Wenn man einen bestimmten Punkt wählt, dann kann man an diesem Punkt eine
Tangente ziehen.  Diese Tangente ist dann die Ableitung der Funktion
\emph{an diesem Punkt}.  Die allgemeine Form einer Ableitung ergibt eine
Funktion, die besagt, was die Steigung einer Tangente an jedem beliebigen
Punkt hat.

\section{Das Ableiten eines Polynoms vom Grad $n$.}
\label{sec:ableiten-grad-n}

Die Ableitung eines Polynoms (z.\,B. $x^n$) findet man, indem man die Potenz
...
\noindent
wobei $f'(x)$ ist die Ableitung der Funktion $f(x)$.
...
Betrachten wir diesen Prozess genauer, indem wir jeden Term einzeln
...
Wobei der Ableitungsprozess auch so dargestellt werden kann:
...
Betrachten wir z.\,B. dieses Diagramm der Funktion $f(x) = 3$ an:

\includegraphics[width=\textwidth]{constant-function.pdf}

Wir erkennen, dass die Funktion keine Steigung hat. Das bedeutet, dass die
Ableitung dieser Funktion gleich 0 (Null) ist.
...
Wobei $m$ (die Steigung der Gerade) entspricht den Wert 10 und $b$ (der
...
Polynoms vom Grad $n$ in Abschnitt~\ref{sec:ableiten-grad-n}:
...
Da $m$ die Steigung der Gerade ist, was dem Wert $-12$ entspricht, kann man
...
(wie aus der Diskussion in Abschnitt~\ref{sec:ableiten-grad-n}), dann
bekommen wir dasselbe Ergebnis:
...
Die Ableitung des zweiten Terms lautet wie folgt.
...
Konstanten ableiten, wie in Abschnitt~\ref{sec:ableitung-konstante}
diskutiert.  Das heißt, dass die Ableitung dieses Terms ist gleich 0 (Null):
